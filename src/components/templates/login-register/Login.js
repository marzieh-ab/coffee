...
const Login = ({showRegisterForm}) => {
  ...
  const loginWithPassword = async () => {
    ...
    console.log("Res ->", res);
    if (res.status === 200) {
      swal({title:"با موفقیت لاگین شدین", icon:"success", buttons:"ورود به پنل کاربری"}).then(()=>{
...
<div className={styles.form}>
        ...
        <button className={styles.btn_light} onClick={showRegisterForm} c >ثبت نام</button>
      </div>
